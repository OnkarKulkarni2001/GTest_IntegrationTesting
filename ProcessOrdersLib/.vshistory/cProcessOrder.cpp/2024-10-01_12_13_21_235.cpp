...
{
	for (vector<cProducts*>::iterator productIterator = this->orderItems.begin(); productIterator != orderItems.end(); productIterator++) {
		if (orderItems[productIterator] == product) {
			true;
			break;
		}
	}
